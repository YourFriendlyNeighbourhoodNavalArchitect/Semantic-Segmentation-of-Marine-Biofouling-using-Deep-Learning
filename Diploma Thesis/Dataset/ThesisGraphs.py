...
class ThesisGraphs:
    ...
    def plotClassOccurrences(self):
        ...
        figure.tight_layout()
        outputPath = self.outputDirectory / 'Class occurence bar chart.png'
    ...
    def plotClassCorrelation(self):
        ...
        axes.tick_params(axis = 'y', labelsize = 12)
        figure.tight_layout()
        outputPath = self.outputDirectory / 'Class correlation matrix.png'
        savefig(outputPath, dpi = 600)
        close()
    ...
    def plotUniqueClassesPerImage(self):
        ...
        for bar, count, percentage in zip(bars, uniqueCountDistribution, percentages):
            ...
            axes.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), label, 
                      ha = 'center', va = 'bottom', fontsize = 10, color = 'black')
        figure.tight_layout()
        outputPath = self.outputDirectory / 'Count of unique classes per image.png'
        savefig(outputPath, dpi = 600)
        close()
