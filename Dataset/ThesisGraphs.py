...
from configurationFile import ALL_PATH, CLASS_DICTIONARY, NUM_CLASSES, VISUALIZATIONS_PATH

class ThesisGraphs:
    def __init__(self, metadataFilePath, outputDirectory):
    ...
    def analyzeMetadata(self):
        ...
        for _, data in self.metadata.items():
    ...
    def plotClassOccurrences(self):
        ...
        classLabels = ['\n'.join(wrap(label, width = 10)) for label in classLabels]
    ...
    def plotClassCorrelation(self):
        ...
        classLabels = ['\n'.join(wrap(label, width = 10)) for label in classLabels]
        figure, axes = subplots(figsize = (12, 10))
...
metadataFilePath = join(ALL_PATH, r'Masks\Metadata.json')
ThesisGraphs(metadataFilePath, VISUALIZATIONS_PATH)