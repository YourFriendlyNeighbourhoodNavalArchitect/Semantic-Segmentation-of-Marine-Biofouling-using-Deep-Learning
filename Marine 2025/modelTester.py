...
from Various.configurationFile import MODEL_PATH, TESTING_PATH, CLASS_DICTIONARY_new
...
class ModelTester:
    ...
    def plotResults(
        ...
        groundTruthLegendLabels, groundTruthHandles = self.generateLegend(
            groundTruthCoverage
        )
        axes[1].legend(
            handles=predictedHandles,
            labels=predictedLegendLabels,
            loc="upper right",
        ...
        axes[2].legend(
            handles=groundTruthHandles,
            labels=groundTruthLegendLabels,
            loc="upper right",
            title="Class Coverage",
            title_fontsize=16,
            fontsize=14,
        ...
        plt.savefig(
            self.outputDirectory / f"Inference {index + 1}.png",
            dpi=600,
            bbox_inches="tight",
        )
        plt.close(figure)
    ...
    def runInference(self):
        ...
        for i in range(len(self.dataset)):
            ...
            aggregatedMetrics = {
                key: value + metrics[key] for key, value in aggregatedMetrics.items()
            }
            self.plotResults(image, np.argmax(output, axis=1).squeeze(), groundTruth, i)

        averagedMetrics = {
            key: value / len(self.dataset) for key, value in aggregatedMetrics.items()
        }
        print(
...
if __name__ == "__main__":
    ...
    device = setupDevice()
    output_directory = MODEL_PATH / "Predictions" / "SimpleUNet"
    ModelTester(
