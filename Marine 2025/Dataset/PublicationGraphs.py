...
class PublicationGraphs:
    def __init__(self, metadataPath: Path, outputDirectory: Path) -> None:
        self.metadataPath = metadataPath
        self.outputDirectory = outputDirectory
        self.metadata = self.loadMetadata()
        self.totalImages = len(self.metadata)
        self.classNames = {
            value["index"]: key for key, value in CLASS_DICTIONARY_new.items()
        }
        ...
        self.classOccurrences = np.zeros(NUM_CLASSES_new, dtype=int)
        self.correlationMatrix = np.zeros(
    ...
    def loadMetadata(self):
        with Path.open(self.metadataPath, "r") as f:
            return load(f)
    ...
    def analyzeMetadata(self):
        ...
        self.correlationMatrix = np.divide(
            self.correlationMatrix,
            rowSums,
            out=np.zeros_like(self.correlationMatrix),
            where=rowSums > 0,
        )
    ...
    def plotClassOccurrences(self):
        ...
        sortedOccurrences = sorted(
            enumerate(self.classOccurrences),
            key=lambda x: x[1],
            reverse=True,
        )
        classIndices, occurrenceCounts = zip(*sortedOccurrences, strict=False)
        ...
        colors = [
            tuple(
                i / 255 for i in CLASS_DICTIONARY_new[self.classNames[index]]["color"]
            )
        ...
        axes.legend(
            handles=[axes.plot([], [])[0]],
            labels=[f"Dataset Size = {self.totalImages}"],
            loc="upper right",
            fontsize=12,
            handletextpad=0,
            handlelength=0,
        ...
        figure.tight_layout()
        outputPath = self.outputDirectory / "Class Occurrence Frequency.pdf"
        savefig(outputPath, dpi=600, format="pdf")
        close()

    def plotClassCorrelation(self):
        # Plot heatmap using precomputed normalized correlation matrix.
        classLabels = [self.classNames[index] for index in range(NUM_CLASSES_new)]
        classLabels = ["\n".join(self.wrapper.wrap(label)) for label in classLabels]
        ...
        axes.tick_params(axis="y", labelsize=12)
        figure.tight_layout()
        outputPath = self.outputDirectory / "Class Correlation Matrix.pdf"
        savefig(outputPath, dpi=600, format="pdf")
        close()
    ...
    def plotUniqueClassesPerImage(self):
        ...
        labels = range(1, NUM_CLASSES_new + 1)
        percentages = uniqueCountDistribution / self.totalImages * 100

        figure, axes = subplots(figsize=(10, 6))
        bars = axes.bar(
            labels,
            uniqueCountDistribution,
            color="skyblue",
            edgecolor="black",
        )
        axes.set_ylabel("Image Count", fontsize=14)
        axes.set_title(
            "Count of Unique Classes per Image",
            fontsize=16,
            fontweight="bold",
        )
        axes.tick_params(axis="x", labelsize=12)
        ...
        figure.tight_layout()
        outputPath = self.outputDirectory / "Count of Unique Classes per Image.pdf"
        savefig(outputPath, dpi=600, format="pdf")
        close()
...
PublicationGraphs(METADATA_PATH, VISUALIZATIONS_PATH)
